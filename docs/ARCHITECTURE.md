...
网页支持点击地图设置目标。画布显示和点击共用坐标变换，适配页面缩放与不同屏幕尺寸；`POST /api/command` 的 `{"command":"goal","x":2,"y":6}` 将米制坐标交给 `NavigationWorld.set_goal()` 校验。目标必须给机器人半径留出障碍与边界间隙，非法请求保留原状态。

改点保留机器人位置、朝向和地图，开始新一段行程并清空轨迹、计时、奖励与控制器记忆。运行中继续运行，暂停或到达后保持暂停；碰撞后必须重置。重置同一地图或切换控制器保留目标，换种子/场景恢复默认目标。训练环境的标准 `reset()` 仍使用原始固定目标，交互演示不改变既有基准任务。

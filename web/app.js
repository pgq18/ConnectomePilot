...
let state=null, scenario='slalom', busy=false, polling=false;
...
function render(){if(!state)return;const w=state.world,t=state.telemetry;
 ...
 const showOverlay=state.paused;$('overlay').hidden=!showOverlay;
 if(showOverlay){$('overlay').querySelector('span').textContent=w.status==='running'?'READY WHEN YOU ARE':'EPISODE COMPLETE';$('overlay').querySelector('strong').textContent=({success:'已抵达。换个场景再试一次。',collision:'碰撞了。保留这个结果，比较下一组。',timeout:'时间结束，机器人尚未抵达。'})[w.status]||(w.steps?'实验已暂停。':'从起点，向目标出发。');$('overlay').querySelector('small').textContent=w.status==='running'?'点击“开始实验”，观察传感和动作变化。':'重置相同场景，可以对比另一种控制器。';}
...
function draw(){if(!state)return;const w=state.world,box=canvas.getBoundingClientRect(),dpr=devicePixelRatio||1;
 ...
 ctx.setTransform(dpr,0,0,dpr,0,0);const W=box.width,H=box.height,pad=23,s=Math.min((W-pad*2)/w.width,(H-pad*2)/w.height),ox=(W-w.width*s)/2,oy=(H-w.height*s)/2;
...
async function poll(){if(polling)return;polling=true;try{const r=await fetch('/api/state');if(!r.ok)throw Error('服务未响应');state=await r.json();render();}catch(e){$('connection').textContent='连接已断开';$('connection').classList.remove('ok');}finally{polling=false;setTimeout(poll,150);}}
